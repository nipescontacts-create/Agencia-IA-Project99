(function () {
  ...
  function renderAIModelsGrid(){
    ...
    const models = (D && D.aiModels) ? D.aiModels : [];
    if(!models.length){
      container.innerHTML='<div class="alert alert--info" style="margin:.5rem 0">No se pudieron cargar los modelos. Recarga la pagina.</div>';
      return;
    }
    const frontier=models.filter(m=>m.category==="Frontier");
    const others=models.filter(m=>m.category!=="Frontier");
